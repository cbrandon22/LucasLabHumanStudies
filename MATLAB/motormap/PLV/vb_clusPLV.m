...
%         freqStruct.labels = {'broadTheta250_lowFreq100','alpha200_beta100';'delta600_theta250';'lowFreq250_highFreq50'};
%         freqStruct.range1 = [3,12;8,12;1,4;3,33];
%         freqStruct.range2 = [3,33;12,32;4,8;70,100];
%         freqStruct.order1 = [250;200;600;250];
%         freqStruct.order2 = [100;100;250;50];
        ...
        for thisFreq=1:length(freqStruct.labels)
            ...
            if ~isempty(currStruct)
                ...
                disp([subj ': running cluster-based PLV analysis... ' currClus ' ' freqStruct.labels{thisFreq}])
                ...
                for i=1:size(plvGammaMove,2);
                    for j=1:size(plvGammaMove,3);
                        m2_normGammaMove(:,i,j)=((plvGammaMove(:,i,j))-mean(plvGammaMove(baseTimeInd1:baseTimeInd2,i,j),1))./std(plvGammaMove(baseTimeInd1:baseTimeInd2,i,j));
                        m2_normThetaMove(:,i,j)=((plvThetaMove(:,i,j))-mean(plvThetaMove(baseTimeInd1:baseTimeInd2,i,j),1))./std(plvThetaMove(baseTimeInd1:baseTimeInd2,i,j));
                        m2_normGammaWait(:,i,j)=((plvGammaWait(:,i,j))-mean(plvGammaWait(baseTimeInd1:baseTimeInd2,i,j),1))./std(plvGammaWait(baseTimeInd1:baseTimeInd2,i,j));
                        m2_normThetaWait(:,i,j)=((plvThetaWait(:,i,j))-mean(plvThetaWait(baseTimeInd1:baseTimeInd2,i,j),1))./std(plvThetaWait(baseTimeInd1:baseTimeInd2,i,j));
                    end
                end
            ...
            % save
                if saveon
                    ...
                    cd(saveDir);
                    save([subj sprintf('_clusPLV_PW_%s.mat',currClus)], 'm2*', 'plv*','PW*','regions','t')  %'avg*','ROIind*','goodROI', 'ROI'
                end
            end
        end

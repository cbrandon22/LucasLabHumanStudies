...
savePlots = true;
savePowSubset = false; %Only saves power and skips everything else
...
for i=1:length(subjList)
    ...
    if ~isempty(eLbl_list_clus1)
        [clus_tPow,tStruct,clusAnat,config_pow] = le_mm_cat_tfPowPhase(subj,eLbl_list_clus1,'motor',[],[],savePowSubset,'clus1');
        if ~savePowSubset
            if ~exist('subjAnat')
                tStruct_lf = tStruct;
                subjAnat = clusAnat;
    ...
    if ~isempty(eLbl_list_clus2)
        [clus_tPow,tStruct,clusAnat,config_pow] = le_mm_cat_tfPowPhase(subj,eLbl_list_clus2,'motor',[],[],savePowSubset,'clus2');
        if ~savePowSubset
            if ~exist('subjAnat')
                tStruct_lf = tStruct;
                subjAnat = clusAnat;
    ...
    if ~isempty(eLbl_list_clus0)
        [clus_tPow,tStruct,clusAnat,config_pow] = le_mm_cat_tfPowPhase(subj,eLbl_list_clus0,'motor',[],[],savePowSubset,'clus0');
        if ~savePowSubset
            if ~exist('subjAnat')
                tStruct_lf = tStruct;
                subjAnat = clusAnat;

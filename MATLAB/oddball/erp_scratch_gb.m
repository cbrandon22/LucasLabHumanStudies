...
trialType2 = {'TARGETHF','TARGETLF'}; % leave empty to only select trialType1
...
overlayReref = 0; % plot rereference result on top of raw signal
    ...
    if ~ismember(files(f).name,{'.','..','sessInfo.mat'})
        ...
        if plotLeadSingleTrials
            ...
            for trial=includeTrials(1):includeTrials(2)
                ...
                cShift = 1.1*max(max(abs(eeg(trial,:,:))));
                if isnan(cShift),disp(['Skipping trial: ' num2str(trial)]);continue;end
                cpos = [];
                bc=0;

function gazeP = CCDTcalibrate(subj,session,targetPix)
...
startInd = find(eyeData(:,4)== 99); %Index of each loop start
endInd = [startInd-1;length(eyeData)];
endInd = endInd(endInd ~= 0); %Index of each loop end
...
for i=1:Ncal % Calibrate voltages for each loop
    ...
    for ii = 1:N
        mV(ii,:) = median(V(floor(V(:,3))==ii,1:2)); % robust Xvolt Yvolt for target
    end

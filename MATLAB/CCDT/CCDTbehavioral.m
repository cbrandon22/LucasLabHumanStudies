...
for i=1:Ncal % Calibrate voltages for each loop
    ...
    V = loopData(loopData(:,3)>0,:); % XY Voltages and target during fixation
    ...
    gazeP(startInd(i):endInd(i),:) = [eyeData(startInd(i):endInd(i),1)*CC(1,1) + eyeData(startInd(i):endInd(i),2)*CC(1,2) + CC(1,3),...
        eyeData(startInd(i):endInd(i),1)*CC(2,1) + eyeData(startInd(i):endInd(i),2)*CC(2,2) + CC(2,3)];
end

% Replay task
% Code here to recrete task screen + gaze position after calibration
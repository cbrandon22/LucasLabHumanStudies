...
for ii = 1:length(Fsine)
    [jk,indsine(ii)] = min(abs(f-Fsine(ii)));
end
ln = zeros(size(x));
blk = 1:Nwin:N;
for ii = 1:length(blk)
...
%     x(ind) = x(ind) - median(x(ind)); % dc offsets can cause problems with fit
    ...
    ln(ind) = exp(i*2*pi*(0:length(ind)-1)'*Fsine/Fs)*AP + exp(-i*2*pi*(0:length(ind)-1)'*Fsine/Fs)*conj(AP); % linear superposition of fitted sine waves
end
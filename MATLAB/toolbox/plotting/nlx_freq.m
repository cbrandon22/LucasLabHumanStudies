...
subj = 'HUP155_i';                  % subject
elec = 56;                          % electrode
freq = [0.5 10000];                   % frequency range (Hz)
...
% Welch's PSD
[pxx,f] = pwelch(data,10*fix(fs),1*fix(fs),logspace(log10(freq(1)),log10(freq(2)),1000),fs); % 10-sec windows, 1-sec overlap, 100 logorithmically-spaced power estimates
...
xlabel('frequency (Hz)'); ylabel('power (\muV^2/Hz)'); title([subj ' elec ' num2str(elec)],'Interpreter', 'none');

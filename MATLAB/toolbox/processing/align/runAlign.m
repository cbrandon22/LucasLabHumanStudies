function [b] = runAlign(samplerate,beh_file,eeg_file,chan_files,log_files,ms_field,isfrei,isegi,moreAccurateAlign)
...
for f = 1:length(eeg_file)
  ...
  if ~moreAccurateAlign
      ...
      disp(sprintf('file %d, alignment method 1: %d matches of %d recorded pulses',f,length(beh_in{f}),length(pulses)));
      % run logalign with the beh_ms and eeg_offset info
      [a,err1,b]=logalign(beh_in,eeg_in,chan_files,log_files,ms_field);
  else
      [beh_in2{f},eeg_in2{f}] = pulsealign2(beh_ms,pulses);
      disp(sprintf('file %d, alignment method 2: %d matches of %d recorded pulses',f,length(beh_in2{f}),length(pulses)));
      [a,err2,b]=logalign(beh_in2,eeg_in2,chan_files,log_files,ms_field);
  end
end

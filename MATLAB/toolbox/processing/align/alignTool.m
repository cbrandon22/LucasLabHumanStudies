...
% Last Modified by GUIDE v2.5 01-Jun-2011 17:35:26
...
%if filenames are provided, load sync data for extraction %AGR 04-24-13
if ~isempty(varargin) && strcmp(varargin{1},'filename')
    loadFile(varargin{2},handles)
end
% Load the default path %IMP 07-23-14
if ~isempty(varargin) && strcmp(varargin{1},'rootPath')
    handles.rootPath=varargin{2};
else
    handles.rootPath='*';
end
...
if isempty(filename) 
  ...
  [filename,pathname] = uigetfile(fullfile(handles.rootPath,'*.*'),'Pick a sync-pulse file','MultiSelect','on');
...
if length(filename) > 1
  ...
  set(handles.panelPulse,'Title',[filename1 ' - ' filename2,'----',fileLength,' minutes']);
  ...
  % set the title
  fileLength = num2str((length(dat)/samplerate)/60);
  set(handles.panelPulse,'Title',[filename,'----',fileLength,' minutes']);
end
...
pulseInfo = setFilter('High',pulseInfo,handles);
...
% executes in button press in pbFilter %
...
if ~isfield(pulseInfo,'filtFreq')
    set(handles.filtFreq,'String','invalid')
    return
end

filtFreq = pulseInfo.filtFreq;
filtMode = pulseInfo.modeHighLow;
sr = pulseInfo.samplerate;
pulseInfo.dat = buttfilt(pulseInfo.origDat,filtFreq,sr,filtMode,4);

cla
set(gca,'NextPlot','Add');
hDat = plot(handles.axesMain,pulseInfo.dat);
hold on
hPulse = plot(handles.axesMain,[],'r*');
hold off
...
% executes in button press in pbUnfilter %
function pbUnfilter_Callback(hObject, eventdata, handles)
% hObject    handle to tbZoom (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

pulseInfo = get(handles.axesMain,'UserData');
if ~isfield(pulseInfo,'origDat')
    return
end

pulseInfo.dat = pulseInfo.origDat;
...
function filtFreq_Callback(hObject, eventdata, handles)
% hObject    handle to filtFreq (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
...
% get filter string
filtString = get(handles.filtFreq,'String');
filtFreq = str2num(filtString);
if isempty(filtFreq)
    set(handles.filtFreq,'String','invalid')
else
    pulseInfo.filtFreq = filtFreq;
end
...
% --- Executes on button press in tbHigh.
function tbHigh_Callback(hObject, eventdata, handles)
% hObject    handle to tbHigh (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
...
% --- Executes on button press in tbLow.
function tbLow_Callback(hObject, eventdata, handles)
% hObject    handle to tbLow (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
...
% set the new mode
pulseInfo = setFilter('low',pulseInfo,handles);

% save any modifications to the data
set(handles.axesMain,'UserData',pulseInfo);


function pulseInfo = setFilter(toggleFilter, pulseInfo, handles)
% Handles the toggling of the high and low buttons
% toggleMode can be one of {'High', 'Low'}
%
%
...
switch toggleFilter
  case 'high'
    ...
    % turn on the correct toggle
    set(handles.tbLow,'Value',0);

  case 'low'
    
    % turn on the correct toggle
    set(handles.tbHigh,'Value',0);
    
end

pulseInfo.modeHighLow = toggleFilter;
...
% set the new mode
setMethod('two',handles);


function setMethod(toggleFilter, handles)
% Handles the toggling of the alignment method buttons
% toggleMode can be one of {'one', 'two'}
%
%
...
switch toggleFilter
  case 'one'
    ...
    % turn on the correct toggle
    set(handles.Method1,'Value',1);
    set(handles.Method2,'Value',0);

  case 'two'
    
    % turn on the correct toggle
    set(handles.Method1,'Value',0);
    set(handles.Method2,'Value',1);
...
% set the mode to pulse align method
setMethod('one',handles);
...
addFilesToListBox('Pick EEG pulse files',handles.lbEegFiles,handles);
...
addFilesToListBox('Pick behavioral pulse files',handles.lbBehFiles,handles);
...
addFilesToListBox('Pick sample channel files',handles.lbChanFiles, handles);
...
addFilesToListBox('Pick Event files',handles.lbEventFiles, handles);
...
%
function addFilesToListBox(dialogStr,lbHandle, handles)
% Helper function 
%
%[filename,pathname] = uigetfile({'*.*','All Files (*.*)'},dialogStr,'MultiSelect','on');
[filename,pathname] = uigetfile(fullfile(handles.rootPath,'*.*'),dialogStr,'MultiSelect','on');
...
% get the samplerate
[samplerate,nBytes,dataformat,gain] = GetRateAndFormat(chan_files{1});
fprintf('Samplerate: %g\n',samplerate);

ms_field = get(handles.txtTimeField,'String');

isfrei = get(handles.cbIsFrei,'Value');
method = get(handles.Method1,'Value');

% call the function
runAlign(samplerate,beh_file,eeg_file,chan_files,log_files,ms_field,isfrei,0,method)
...
% --- Executes during object creation, after setting all properties.
function filtFreq_CreateFcn(hObject, eventdata, handles)
% hObject    handle to filtFreq (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called

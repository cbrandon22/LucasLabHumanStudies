...
if any(numValid(1:end-1)~=samplesPerRecord) 
  warning('some non-full blocks!');
end
...
info.ADChannel=regexp_grab_token(headerString,'ADChannel\s+([\d\\.]+)');
info.InputRange=regexp_grab_token(headerString,'InputRange\s+([\d\\.]+)');
info.DspLowCutFrequency=regexp_grab_token(headerString,'DspLowCutFrequency\s+([\d\\.]+)');
info.DspHighCutFrequency=regexp_grab_token(headerString,'DspHighCutFrequency\s+([\d\\.]+)');
...
if ~isempty(strfind(headerString,'Pegasus'))
    dateString=regexp_grab_token(headerString,'TimeCreated ([\d/]+)');
    pat='TimeCreated [\d/]+ ([\d:\.])+';

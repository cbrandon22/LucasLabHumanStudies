...
%  (3) updated by CLB 10/17
%              -function now add all non-TTL neuralynx events to a separate
%              structure (nlxEvents.mat) from Events.nev
% To-Do:
...
fid = fopen(eegPulseFile);
eegPulse_ms = textscan(fid,'%f');
eegPulse_ms = eegPulse_ms{1}./1e3;
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [meddev,rawDevs]=logalign_local(beh_ms,nlx_ms,evCellArray,samplerate,lfp_samplerate,first_sample_time,fullFileExt)
  ...
  for e=1:numEventsToAlign
    ...
    if e==2
        nevEvents = ExtractNEVfileInfo(thisEv);
        evCount = 1;
        for i=1: length(nevEvents)
            if ~strcmp(nevEvents(i).note{1}(1:3),'TTL')
                thisNlxEvTime = nevEvents(i).timestamp/1e3;
                nlxEvents(evCount).subj = subj;
                nlxEvents(evCount).session = sess;
                nlxEvents(evCount).lfpmsTime = thisNlxEvTime;
                nlxEvents(evCount).type = nevEvents(i).note{1};
                nlxEvents(evCount).lfpfile = FULLFILEEXT;
                nlxEvents(evCount).lfpoffset = round((thisNlxEvTime-(first_sample_time/1000))*(lfp_samplerate/1000)); %in lfp samples 
                nlxEvents(evCount).timesfile = TIMESFILE;
                nlxEvents(evCount).timesoffset = thisNlxEvTime;
                evCount = evCount+1;
            end
        end
        sessDir = evCellArray{1}(1:strfind(evCellArray{1},'events.mat')-1);
        save([sessDir 'nlxEvents.mat'],'nlxEvents');
        continue;
    end

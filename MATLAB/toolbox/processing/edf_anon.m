function [EDF_files, PIDs] = edf_anon(edf_dir, show_name)
%
% DESCRIPTION:
...
if ~exist('show_name','var') || isempty(show_name)
    show_name = false
end
...
for k=1:length(d)
  ...
  EDF_files{end+1} = EEG_file;
  PIDs{end+1} = anonThisFile_local(EEG_file, show_name);
end

function PID = anonThisFile_local(EEG_file, show_name)

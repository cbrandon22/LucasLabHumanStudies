function filestem = edf_split(eegfile,subject,outputdir,badLabels)
...
%
%
%JJ: on 3/30/2009 I made some updates to the code that prints the sample rate, because it was
%printing out our jefferson data as sampled at 40 Hz instead of 400 Hz.
%
%JFB: on 6/25/2013 now it checks to see if the jacksheet is already
%made.  if it is, it checks to see if the current one is the same
%as the one that was made previously
%
%AGR: on 6/29/2014 Updated how the jacksheet labels are written so it is
%compatible with voxTool (no need for re-formatting it using
...
%JJ 3/30/2009: JJ added the next line, to replace the previous one, to account for the fact that the sample rate from the data exported from jeff was being incorrectly printed as 40 Hz, instead of 400Hz
fprintf(fout1,'samplerate %.2f\ndataformat ''%s''\ngain %g\n',SampleRate,outdataformat,mode(Cal));
fclose(fout1);
...
jackFile_OLD = fullfile(outputdir,'jacksheet.txt');
jackFile_NEW = fullfile(outputdir,sprintf('%s.jacksheet.txt',name));
if exist(jackFile_NEW,'file')
  fprintf('\n\t%s already made (split previously?)..EXITING\n\n',jackFile_NEW)
  return
end
fout3 = fopen(jackFile_NEW,'w','l');
fprintf(' Making the jacksheet..... ')
ekgcount = 0;
for c = 1:size(Label,1)
  thisLabel = Label(c,:);
  whtSpace = find(thisLabel==' ');
  while whtSpace(2)-whtSpace(1) >1; % remove whitespace within label
      thisLabel(whtSpace(1)) = '';
      whtSpace = find(thisLabel==' ');
  end
  if ~isempty(regexp(thisLabel,'EEG '))
    fprintf('\n\tWARNING: removing ''EEG '' in %s',['''' thisLabel '''']);
    thisLabel = regexprep(thisLabel,'EEG ','');
  end
  if ~isempty(regexp(thisLabel,'_0'))
    fprintf('\n\tWARNING: removing ''_0 '' in %s',['''' thisLabel '''']);
    thisLabel = regexprep(thisLabel,'_0','');
  end
  if ~isempty(regexp(thisLabel,'-REF'))
    fprintf('\n\tWARNING: removing ''-REF '' in %s',['''' thisLabel '''']);
    thisLabel = regexprep(thisLabel,'-REF','');
  end
  if ~isempty(regexp(thisLabel,'EKG'))
      ekgcount = ekgcount+1;
      thisLabel = ['EKG' num2str(ekgcount)];
      fprintf('\n\tWARNING: reformating EKG label');
  end
  % This step excludes badLabels from the jacksheet
  if ismember(strtrim(thisLabel),badLabels)
      %any(~cellfun(@isempty,regexp(thisLabel,badLabels)))
     skipchannel(c) = 1; % used below when writing eeg data
     continue;
  else 
      skipchannel(c) = 0;
  end
...
% make sure the jacksheets match
fout3 = fopen(jackFile_NEW,'r');
X = textscan(fout3,'%d%s');
fclose(fout3);
elecNum_NEW = X{1};
elecNam_NEW = X{2};
if ~exist(jackFile_OLD,'file')
  fprintf('\n')
  fprintf(' Making the mother jacksheet....')
  system(['cp ' jackFile_NEW ' ' jackFile_OLD]);
  pause(.1)
  fprintf(' done\n')
end
fout4 = fopen(jackFile_NEW,'r');
X = textscan(fout4,'%d%s');
fclose(fout4);
elecNum_OLD = X{1};
elecNam_OLD = X{2};
if ~isequal(elecNum_NEW,elecNum_OLD)||~isequal(elecNam_NEW,elecNam_OLD)
  error(sprintf('\n\n\tERROR: %s does not match saved jacksheet',jackFile_NEW))
end
...
for c = chan
  % skip writing channels that were left off the jacksheet
  if skipchannel(c)
      continue
  end
  fprintf('%d ',c);

% behavioral data extraction - LOC gives time of max RT, equivalent to loss
% of consciousness; events output gives event time chunk of this LOC for
% input into downstream LFP data sorting

% author ehudgins@gmail.com 2016

function [LOC,events] = behavee(ddir,subj)
dobehave = 1;
...
%     ret = xRT1(seq(1)); % time of max RT for resorting
    ...
    retsr = ret*timefix;
    lfpoffset = [events(1:end).lfpoffset].';
    evn = find(lfpoffset > retsr);
    LOC = evn;
    %LOC = events(evn(1)).lfpoffset; % LOC time in seconds from max RT
end

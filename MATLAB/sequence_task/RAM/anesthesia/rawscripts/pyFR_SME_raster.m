function pyFR_SME_raster(subj,rawDataPath,behDataPath,channelStr)
...
%behDir  = fullfile(subjDir,'behavioral/pyFR_stim2',behDataPath);

% load events
evFile  = fullfile(behDir,'events.mat');
...
wordEv  = ev(strcmp({ev.type},'WORD'));
recEv   = wordEv([wordEv.recalled]==1);
notEv   = wordEv([wordEv.recalled]==0);

% get lfp file info
[lfpDir,fileExt]=fileparts(wordEv(1).lfpfile);
...
for k=1:length(unClusters)
  ...
  % plot the rasters
  axRec = plotRaster(rastRec,tVect,'r',TS,[recEv.serialpos],1);
  axNot = plotRaster(rastNot,tVect,'b',TS,[notEv.serialpos],2);
...
function ax=plotRaster(rast,tVect,col,Ts,sp,fNum)
  ...
  set(ax,'YTIckLabel',sp(1:4:length(sp)))  

...
%                    to the ref# 
%
if downsampleStruct.recType == 'pegasus'
    yor = 1; % add to datestr
else
    yor = 0;
end
...
while true
  ...
  elseif ~isempty(regexp(N,startRecCommand))||~isempty(regexp(N,stopRecCommand));;
    ...
    if ~isempty(regexp(N,startRecCommand))
        if downsampleStruct.recType == 'pegasus'
            timeStr = getTimeStr_local2(X{3}{1});
        else
  ...
  elseif ~isempty(regexp(N,timeStampLineMarker))
    ...
    if downsampleStruct.recType == 'pegasus'
        dateStr = getDateStr_local2(X{4+yor}{1});
        fprintf('  Recording Date: %s\n',X{4+yor}{1});
    else
        dateStr = getDateStr_local(X{4+yor}{1});
    fprintf('  Recording Date: %s\n',X{4+yor}{1});
    end
...
function dateStr = getDateStr_local(dateStr_in);
  ...
  function dateStr = getDateStr_local2(dateStr_in);
  ind = regexp(dateStr_in,'/');
  y_str  = dateStr_in(3:ind(1)-1);
  m_str  = num2str(nonzeros(str2num(dateStr_in(ind(1)+1:ind(2)-1))));
  d_str  = num2str(nonzeros(str2num(dateStr_in(ind(2)+1:end)))); 
...
function timeStr = getTimeStr_local(timeStr_in);
  ...
  function timeStr = getTimeStr_local2(timeStr_in);
  ind = regexp(timeStr_in,':');
  hour_str = num2str(nonzeros(str2num(timeStr_in(12:ind(1)-1))));
  min_str  = timeStr_in(ind(1)+1:ind(2)-1);
  hour_str = makeSureStrIsATwoDigitInteger_local(hour_str);
  min_str  = makeSureStrIsATwoDigitInteger_local(min_str);     
  timeStr  = [hour_str min_str];

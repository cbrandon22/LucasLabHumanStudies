...
subj = 'HUP117_i'; % subject
subjF = 'HUP117_i_08Jun16_1708'; % raw data name?
save_dir = [top_dir, subj, '/processed_data/dci'];
notes_dir = [top_dir, subj,  '/docs/'];
data_dir = [top_dir, subj, '/raw/2016-06-08_17-02-24'];

% about elecs
[~, labels, ~] = xlsread([notes_dir, 'electrodeMap(unfinished).xlsx']); % not sure how cell naming scheme works, so read in everything then select what you want
...
dur = events(end).lfpoffset/srate*1000; % offset of lasat event, in ms
...
% Here, propofol was strted before the recording, so ana_start is 1
% start of anasthesia:
...
% differently). Need to subtract timepoint for the first pulse
% start time - first pulse
...
elec_info.all = elecs;
elec_info.bad = sort([4 10 11 12 24 30 31 44 49:52 54:56 58 61:64]);

...
subj = 'HUP138_i'; % subject
subjF = 'HUP138_i_01Jul16_0822'; % raw data name?
save_dir = [top_dir, subj, '/processed_data/dci'];
notes_dir = [top_dir, subj,  '/processed/HUP138_i/docs/'];
data_dir = [top_dir, subj, '/processed/HUP138_i/raw/2017-04-21_07-04-47'];
...
ev_srate = srate; % this subject doesn't hve params thing in the right place
%iso = []; % get this from .nev file

% events
load([top_dir, subj, '/processed/HUP138_i/behavioral/session_0/events.mat']); % get behavioral data
...
% load nev
load(['/Users/tnl/Desktop/C/data/eeg/HUP138_i/', subj, 'NEV.mat']);
% get neuralynx srate
% duration of exp in seconds: taken from header of NEV file
nl_dur = ((8*60*60) + (17*60) + 16) - ((7*60*60) + (5*60) + 23);% srate = total samples/ total seconds
...
ana_start = 799634800/nl_srate; %- 125250496; % HUP121i
...
elec_info.all = elecs;
elec_info.bad = sort([61:64 82 85 97 113]);

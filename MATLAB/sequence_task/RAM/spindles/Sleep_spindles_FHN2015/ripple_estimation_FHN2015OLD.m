function [spindles_start_end, detected_spindles] = ripple_estimation_FHN2015(x, fs, spindle_frequency_range, alg_used)
%
...
if nargin<3 || isempty(spindle_frequency_range), 
    spindle_frequency_range = 100:250; % spindle frequencies over which to search for the spindles
end

%% Resample the signal at 100 Hz and run the main function
new_fs = 100; x2 = resample(x, new_fs, fs);
[spindles_start_end, detected_spindles, coefs] = main_spindles_function(x2, new_fs, spindle_frequency_range, alg_used);
...
function [spindles_start_end, detected_spindles, coefs] = main_spindles_function(x, fs, spindle_frequency_range, alg_used)
...
min_spindle_duration = 0.005; max_spindle_duration = 0.3; % expected RIPPLE duration in seconds 10-300ms
...
scales = 0.25:0.1:7; %  2:0.1:15; % scales corresponding to pseudo-frequencies F = scal2frq(0.25:0.01:1.5,'morl',1/100); will give 50-325Hz
